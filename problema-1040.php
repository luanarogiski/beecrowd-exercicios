...
$n1 = (float)$linhaValores[0];
$n2 = (float)$linhaValores[1];
$n3 = (float)$linhaValores[2];
$n4 = (float)$linhaValores[3];

$media = (($n1 * 2) + ($n2 * 3) + ($n3 * 4) + ($n4 * 1)) / 10;

if ($media != (int)$media) {
    $mediaValores = explode('.', $media);
    $media = $mediaValores[0] + (float) ('0.' . substr($mediaValores[1], 0, 1));
}
...
if ($media >= 7.0) {
    ...
    echo 'Nota do exame: ' . number_format($notaExame, 1, '.', '') . PHP_EOL;
    if ($mediaFinal >= 5.0) {
        echo "Aluno aprovado.\n";
    } else {
